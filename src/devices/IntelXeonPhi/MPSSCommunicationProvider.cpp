...
unsigned MPSSCommunicationProvider::getCurrentPowerLimit( void ) const {
	...
	mic_get_power_limit( deviceHandle.get(), &powerLimit );
	std::shared_ptr<mic_power_limit> powerLimitRAIIGuard{ powerLimit, []( mic_power_limit* ptr ) { mic_free_power_limit( ptr ); } };

	std::uint32_t limit;
	MPSS_ERROR_CHECK( mic_get_power_phys_limit( powerLimit, &limit ) );
	MPSS_ERROR_CHECK( mic_get_power_hmrk( powerLimit, &limit ) ); //power limit 0
	MPSS_ERROR_CHECK( mic_get_power_lmrk( powerLimit, &limit ) ); //power limit 1

//	std::uint32_t window;
//	MPSS_ERROR_CHECK( mic_get_time_window0( powerLimit, &window );
//	MPSS_ERROR_CHECK( mic_get_time_window1( powerLimit, &window );
...
void MPSSCommunicationProvider::setPowerLimit( unsigned milliwatts ) {
	...
	MPSS_ERROR_CHECK( mic_set_power_limit0( dev, hmrk, TIME_WINDOW_0 ) );
	MPSS_ERROR_CHECK( mic_set_power_limit1( dev, lmrk, TIME_WINDOW_1 ) );
}
...
devices::DeviceIdentifier::idType MPSSCommunicationProvider::getPrimaryId( int index ) {
	...
	MPSS_ERROR_CHECK( mic_get_uuid( devPtr.get(), UUID, &size ) != E_MIC_SUCCESS );
...
std::map<std::string, std::string> MPSSCommunicationProvider::getInfo( int index ) {
	...
	std::uint8_t UUID[ MAX_NAME_LENGTH ];
	size_t size = MAX_NAME_LENGTH;
	mic_get_uuid( dev, UUID, &size );
	info[ "UUID" ] = reinterpret_cast<char*>( UUID );

	info[ "Name" ] = mic_get_device_name( dev );

	char stringBuf[ MAX_NAME_LENGTH ];
	MPSS_ERROR_CHECK( mic_get_silicon_sku( dev, stringBuf, &size ) );
	info[ "SiliconSKU" ] = stringBuf;

	MPSS_ERROR_CHECK( mic_get_serial_number( dev, stringBuf, &size ) );
	info[ "SerialNumber" ] = stringBuf;

	mic_cores_info* coresInfo;
	MPSS_ERROR_CHECK( mic_get_cores_info( dev, &coresInfo ) );
	std::shared_ptr<mic_cores_info> coresInfoRAIIGuard{ coresInfo, []( mic_cores_info* ptr ) { mic_free_cores_info( ptr ); } };

	std::uint32_t coresCount;
	MPSS_ERROR_CHECK( mic_get_cores_count( coresInfo, &coresCount ) );
...
void MPSSCommunicationProvider::fillVersionInfo( std::map<std::string, std::string>& info, mic_device* dev ) {
	...
	MPSS_ERROR_CHECK( mic_get_version_info( dev, &version ) );
	std::shared_ptr<mic_version_info> versionRAIIGuard{ version, []( mic_version_info* ptr ) { mic_free_version_info( ptr ); } };

	char stringBuf[ MAX_NAME_LENGTH ];
	size_t size = MAX_NAME_LENGTH;
	MPSS_ERROR_CHECK( mic_get_flash_version( version, stringBuf, &size ) );
	info[ "FlashVersion" ] = stringBuf;

	MPSS_ERROR_CHECK( mic_get_uos_version( version, stringBuf, &size ) );
...
void MPSSCommunicationProvider::fillProcessorInfo( std::map<std::string, std::string>& info, mic_device* dev ) {
	...
	MPSS_ERROR_CHECK( mic_get_processor_info( dev, &processorInfo ) );
	std::shared_ptr<mic_processor_info> processorInfoRAIIGuard{ processorInfo, []( mic_processor_info* ptr ) { mic_free_processor_info( ptr ); } };

	std::uint16_t model, model_ext;
	MPSS_ERROR_CHECK( mic_get_processor_model( processorInfo, &model,	&model_ext ) );
	info[ "ProcessorModel" ] = std::to_string( model );
	info[ "ProcessorModelExtension" ] = std::to_string( model_ext );

	std::uint16_t type;
	MPSS_ERROR_CHECK( mic_get_processor_type( processorInfo, &type ) );
	info[ "ProcessorType" ] = std::to_string( type );

	std::uint32_t id;
	MPSS_ERROR_CHECK( mic_get_processor_steppingid( processorInfo, &id ) );
	info[ "ProcessorSteppingId" ] = std::to_string( id );

	char stringBuf[ MAX_NAME_LENGTH ];
	size_t size = MAX_NAME_LENGTH;
	MPSS_ERROR_CHECK( mic_get_processor_stepping( processorInfo, stringBuf, &size ) );
...
void MPSSCommunicationProvider::fillPciConfigInfo( std::map<std::string, std::string>& info, mic_device* dev ) {
	...
	MPSS_ERROR_CHECK( mic_get_pci_config( dev, &pciConfig ) );
	std::shared_ptr<mic_pci_config> pciConfigRAIIGuard{ pciConfig, []( mic_pci_config* ptr ) { mic_free_pci_config( ptr ); } };

	std::uint16_t vendorId;
	MPSS_ERROR_CHECK( mic_get_vendor_id( pciConfig, &vendorId ) );
	info[ "VendorId" ] = std::to_string( vendorId );

	std::uint16_t deviceId;
	MPSS_ERROR_CHECK( mic_get_device_id( pciConfig, &deviceId ) );
	info[ "DeviceId" ] = std::to_string( deviceId );

	std::uint8_t revisionId;
	MPSS_ERROR_CHECK( mic_get_revision_id( pciConfig, &revisionId ) );
	info[ "RevisionId" ] = std::to_string( revisionId );

	std::uint16_t subsystemId;
	MPSS_ERROR_CHECK( mic_get_subsystem_id( pciConfig, &subsystemId ) );
	...
	MPSS_ERROR_CHECK( mic_get_link_width( pciConfig, &linkWidth ) );
	info[ "LinkWidth" ] = std::to_string( linkWidth );

	std::uint32_t payload;
	MPSS_ERROR_CHECK( mic_get_max_payload( pciConfig, &payload ) );
	info[ "MaxPayload" ] = std::to_string( payload );

	std::uint32_t readreq;
	MPSS_ERROR_CHECK( mic_get_max_readreq( pciConfig, &readreq ) );
	info[ "MaxReadRequests" ] = std::to_string( readreq );

	char stringBuf[ MAX_NAME_LENGTH ];
	size_t size = MAX_NAME_LENGTH;
	MPSS_ERROR_CHECK( mic_get_link_speed( pciConfig, stringBuf, &size ) );
...
void MPSSCommunicationProvider::fillMemoryInfo( std::map<std::string, std::string>& info, mic_device* dev ) {
	...
	MPSS_ERROR_CHECK( mic_get_memory_info(dev, &memoryInfo ) );
	std::shared_ptr<mic_device_mem> memoryInfoRAIIGuard{ memoryInfo, []( mic_device_mem* ptr ) { mic_free_memory_info( ptr ); } };

	char stringBuf[ MAX_NAME_LENGTH ];
	size_t size = MAX_NAME_LENGTH;
	MPSS_ERROR_CHECK( mic_get_memory_vendor( memoryInfo, stringBuf, &size ) );
	info[ "LinkSpeed" ] = stringBuf;

	std::uint32_t revision;
	MPSS_ERROR_CHECK( mic_get_memory_revision( memoryInfo, &revision ) );
	info[ "MemoryRevision" ] = std::to_string( revision );

	std::uint32_t density;
	MPSS_ERROR_CHECK( mic_get_memory_density( memoryInfo, &density ) );
	info[ "MemoryDensity" ] = std::to_string( density );

	std::uint32_t memorySize;
	MPSS_ERROR_CHECK( mic_get_memory_size( memoryInfo, &memorySize ) );
	info[ "MemorySize" ] = std::to_string( memorySize );

	size = sizeof( stringBuf );
	MPSS_ERROR_CHECK( mic_get_memory_type( memoryInfo, stringBuf, &size ) );
	info[ "MemoryType" ] = stringBuf;

	std::uint32_t frequency;
	MPSS_ERROR_CHECK( mic_get_memory_frequency( memoryInfo, &frequency ) );
	info[ "MemoryFrequency" ] = std::to_string( frequency );

	std::uint16_t ecc;
	MPSS_ERROR_CHECK( mic_get_ecc_mode( memoryInfo, &ecc ) );
...
void MPSSCommunicationProvider::checkMPSSErrors( const char* source, int status ) {
	if ( status != E_MIC_SUCCESS ) {
		...
		err << mic_get_error_string() << ": " << strerror( errno );
...
MPSSCommunicationProvider::DevicesListWrapper MPSSCommunicationProvider::getDevicesList( void ) {
	...
	auto result = mic_get_devices( &micDevicesList );
	...
	std::shared_ptr<mic_devices_list> micDevicesListRAIIGuard{ micDevicesList, []( mic_devices_list* ptr ) { mic_free_devices( ptr ); } };

	int devicesCount;
	MPSS_ERROR_CHECK( mic_get_ndevices( micDevicesList, &devicesCount ) != E_MIC_SUCCESS);
...
MicDevicePtr MPSSCommunicationProvider::getDeviceByIndex( int index ) {
	...
	int deviceHandle;
	MPSS_ERROR_CHECK( mic_get_device_at_index( micDevicesListWrapper.getPtr(), index, &deviceHandle ) != E_MIC_SUCCESS );

	mic_device* device;
	MPSS_ERROR_CHECK( mic_open_device( &device, deviceHandle ) != E_MIC_SUCCESS );

	auto deviceDeleter = []( mic_device* ptr ) { mic_close_device( ptr ); };
